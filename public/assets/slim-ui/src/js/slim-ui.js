	...
	function setupAutocompletes() {
		...
		fields.forEach((field) => {
			...
			if (field.dataset.ajaxUrl) {
				sendRequest = (query) => {
					let ajaxUrl = field.dataset.ajaxUrl;

					if (ajaxUrl.includes('{query}')) {
						ajaxUrl = ajaxUrl.replace('{query}', query);
					} else {
					...
					ajax.send();
					ajax.onload = () => {
						const r = JSON.parse(ajax.responseText);
						const list = r.message ? r.data : r;
